...
class LoginForm(forms.Form):
    ...
    def clean(self):
        ...
        if user is None:
            ...
            raise forms.ValidationError('你小子账号或密码输错了')
        else:
            self.cleaned_data['user'] = user
        return self.cleaned_data
...
class RegForm(forms.Form):
    username = forms.CharField(label='用户名', min_length=4, max_length=30, widget=forms.TextInput(
    ...
    verification_code = forms.CharField(label='验证码', required=False, widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': '发送验证码'}))

    password = forms.CharField(label='密码', min_length=4, widget=forms.PasswordInput(
        attrs={'class': 'form-control', 'placeholder': '请输入密码'}))

    password_again = forms.CharField(label='确认密码', min_length=4, widget=forms.PasswordInput(
    ...
    def clean_username(self):
        ...
        if User.objects.filter(username=username).exists():
            raise forms.ValidationError('名字重复啦换一个吧')
        return username

    def clean_email(self):
        email = self.cleaned_data['email']
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError("邮箱重复啦换一个吧")
        return email
...
class BindEmailForm(forms.Form):
    email = forms.EmailField(
        label='邮箱',
        widget=forms.EmailInput(
            attrs={'class': 'form-control', 'placeholder': '输入正确的邮箱'}
        )
    )
    verification_code = forms.CharField(
        label='验证码',
    ...
    def clean_email(self):
        email = self.cleaned_data['email']
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError('该邮箱已经被绑定过了哦')
        return email
